...
export function getMT5Client(baseUrl?: string, userId?: string): MT5ClusterClient {
  if (!clientInstance && baseUrl && userId) {
    clientInstance = new MT5ClusterClient(baseUrl, userId);
  }
  if (!clientInstance) {
...
export function initMT5Client(baseUrl: string, userId: string): MT5ClusterClient {
  clientInstance = new MT5ClusterClient(baseUrl, userId);
  return clientInstance;
}
