...
int OnInit()
{
   ...
   //--- Initialize ZeroMQ first (REQUIRED - will not start without libzmq.dll)
   if(!InitializeZMQ())
   {
      g_statusMessage = "ERROR: ZeroMQ failed - ensure libzmq.dll is in MQL5/Libraries/";
      UpdateComment();
      Print(g_statusMessage);
      Print("  Download libzmq.dll (x64) from: https://github.com/zeromq/libzmq/releases");
      Print("  Place in: <MT5 Data Folder>/MQL5/Libraries/libzmq.dll");
      Alert("Hedge Edge: libzmq.dll not found in MQL5/Libraries/ - see Experts tab");
      return INIT_FAILED;
   }
   ...
   //--- Initialize License DLL
   bool dllAvailable = InitializeDLL();
   
   if(!dllAvailable)
   {
      Print("WARNING: HedgeEdgeLicense.dll not found in MQL5/Libraries/");
      Print("  License validation will use WebRequest fallback");
      
      if(!InpDevMode)
      {
         //--- In production: try WebRequest license validation
         if(StringLen(InpLicenseKey) == 0)
         {
            g_statusMessage = "ERROR: License Key is required";
            UpdateComment();
            Print(g_statusMessage);
            Alert("Hedge Edge: Please enter your License Key in EA settings");
            return INIT_PARAMETERS_INCORRECT;
         }
         
         //--- Validate via WebRequest
         if(!ValidateLicenseViaWebRequest())
         {
            g_statusMessage = "License validation failed: " + g_lastError;
            UpdateComment();
            Print(g_statusMessage);
            // Don't fail - allow retry on timer
         }
         else
         {
      ...
      {
         //--- DEV MODE: DLL not available but dev mode allows testing
   ...
   {
      //--- DLL available: always validate license
      ...
      {
         //--- DEV MODE with DLL but no key
         Print("*** DEV MODE: DLL loaded, license check skipped ***");
         g_isLicenseValid = true;
      }
   }
   ...
   g_statusMessage = g_isLicenseValid ? 
      (InpDevMode ? "DEV MODE - Active (ZMQ)" : "Licensed - Active (ZMQ)") :
      "Awaiting License Validation";
...
void GatherPositions()
{
   ...
   for(int i = 0; i < totalPositions; i++)
   {
      if(PositionSelectByTicket(PositionGetTicket(i)))
      {
         ...
         g_positions[i].commission = 0; // Commission tracked via deals, not positions in newer MT5
...
//+------------------------------------------------------------------+
//| Validate license via WebRequest (DLL-free fallback)                |
//+------------------------------------------------------------------+
bool ValidateLicenseViaWebRequest()
{
   if(StringLen(InpLicenseKey) == 0)
   {
      g_lastError = "License key not provided";
      return false;
   }
   
   string url = InpEndpointUrl;
   string headers = "Content-Type: application/json\r\n";
   
   string accountId = IntegerToString(AccountInfoInteger(ACCOUNT_LOGIN));
   string broker = AccountInfoString(ACCOUNT_COMPANY);
   
   string body = "{\"licenseKey\":\"" + InpLicenseKey + "\","
                 + "\"accountId\":\"" + accountId + "\","
   ...
   int httpCode = WebRequest("POST", url, headers, timeout, postData, resultData, resultHeaders);
   
   if(httpCode == -1)
   {
      int err = GetLastError();
      if(err == 4060)
         g_lastError = "WebRequest not allowed. Add " + url + " to Tools > Options > Expert Advisors > Allow WebRequest";
      else
         g_lastError = "WebRequest failed, error: " + IntegerToString(err);
      Print("License WebRequest error: ", g_lastError);
      return false;
   }
   ...
   if(httpCode == 200)
   {
      ...
      string valid = ExtractJsonValue(response, "valid");
      
      if(valid == "true")
      {
         g_lastLicenseCheck = TimeCurrent();
         Print("License validated via WebRequest for account: ", accountId);
         return true;
      }
      else
      {
         ...
         if(StringLen(g_lastError) == 0)
            g_lastError = "License rejected by server";
         return false;
      }
   }
   
   g_lastError = "HTTP " + IntegerToString(httpCode) + " from license server";
   return false;
}
...
string OpenPositionByCommand(string request)
{
   ...
   //--- Filling mode (try IOC first, then FOK, then RETURN)
