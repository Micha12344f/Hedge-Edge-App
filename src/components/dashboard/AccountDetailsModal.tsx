...
import { checkVPSHealth } from "@/lib/vps-mt5-api";
import type { MT5Position } from "@/lib/vps-mt5-api";
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
export function AccountDetailsModal({
  ...
  // Check for cached password and VPS status when modal opens
  useEffect(() => {
    if (open && account?.login && account?.server) {
      ...
      checkVPSHealth()
        .then(result => setVpsStatus(result.status === 'healthy' ? 'online' : 'offline'))
        .catch(() => setVpsStatus('offline'));
    }
  }, [open, account?.login, account?.server]);
  ...
  const {
    ...
    enabled: open && !needsPassword && !!cachedPassword && vpsStatus === 'online',
    pollInterval: 3000,
    fullSnapshot: true,
  });
      ...
      <SheetContent className="w-full sm:max-w-xl md:max-w-2xl overflow-hidden flex flex-col">
        <SheetHeader className="flex-shrink-0">
          ...
          <div className="flex items-center justify-between p-2 bg-muted/50 rounded-lg mt-2">
            <div className="flex items-center gap-2">
              ...
              {/* MT5 Connection */}
              {!needsPassword && vpsStatus === 'online' && (
                isConnected ? (
                  <Badge
                    variant="outline"
        ...
        <ScrollArea className="flex-1 mt-4">
          <div className="space-y-4 pr-4">
            ...
            {/* Loading State */}
            {isLoading && !snapshot && !needsPassword && vpsStatus === 'online' && (
              <div className="flex flex-col items-center justify-center p-8 space-y-4">
                <Loader2 className="h-8 w-8 animate-spin text-primary" />
                <p className="text-muted-foreground">
            ...
            {/* Error State */}
            {error && !snapshot && !needsPassword && vpsStatus === 'online' && (
            ...
            {(snapshot || (!needsPassword && !error)) && vpsStatus === 'online' && (
              <>
